...
async def start_command(client, message):
    ...
    buttons = [
        ...
        [InlineKeyboardButton("🔗Repo🔗", url="https://github.com/pachax001/My-DramaList-Bot")]
    ]
    reply_markup = InlineKeyboardMarkup(buttons)
    await message.reply_text(
        "Hi! I can extract details from mydramalist URL or from a search query. For url send /url {mydramalistURL}. For Query send /s {search query}. Join @kdramasmirrorlog for ongoing K-Dramas",reply_markup=reply_markup
    )

    logging.info(f"User {user_id} start the bot")
